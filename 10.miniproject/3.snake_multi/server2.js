...
function moveSnake(clientId) {
  ...
  // FIXME: 데이터가 없거나, 뱀이 비어있을 경우 스킵
  if (!client || !client.data || !Array.isArray(client.data.snake) || client.data.snake.length === 0) {
    console.warn(`Invalid data for client: ${clientId}, skipping this step.`);
    return;
  }
